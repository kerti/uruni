...
10. **Source files are ASCII.** Every file a tool parses — Go, TS/TSX, SQL, shell, YAML, JSON, CSS, `Makefile`, `Dockerfile`, `Caddyfile`, the git hooks — contains no byte above `0x7F`, **comments included**. This is not typography policing. One `§` in a SQL comment made `sqlc generate` silently truncate several *unrelated* queries in the same file (`GROUP BY dues_period` → `dues_perio`, `kind = 'opening'` → `'opening`), because the generator addresses its input by byte offset while counting in runes: it compiled, it ran, and the queries were wrong. Nothing about that failure is specific to sqlc.
    **If a character must reach a human, write it as the language's own escape** — `'\u00a0'` in TypeScript, `"\u2728 Added"` in double-quoted YAML — which keeps the bytes ASCII and the rendered output identical. Otherwise use ASCII: `-` not `—`, `...` not `…`, `->` not `→`, `section` not `§`.
    **Exempt, and only these:** everything under `docs/`, every `*.md`, `LICENSE` and `NOTICE` (third-party legal text), and `web/src/copy/id.ts` — the one designated copy surface, where Indonesian typography is the treasurer's reading experience and no generator parses the file. `TestSourceFilesAreASCII` (repo root) enforces all of this under `make check`.
...
- Frontend lint is **oxlint**, not ESLint: `npm run lint` → `oxlint` (Tailwind v4 via `@tailwindcss/vite`).
- `store/` is **generated by sqlc** (`make sqlc`) from `internal/db/migrations` + `internal/store/queries` — the migrations *are* the schema, one copy, no drift. Regenerate, never hand-edit the output.
- One engine, SQLite, through `0.x` (ADR-004/005); schema is a **single file** `internal/db/migrations/00001_schema.sql` edited in place — after pulling a schema change run `make db-reset`, never `make migrate-up` (goose won't re-apply an edited file; ADR-025).
- **Source files are ASCII** (rule 10) — `TestSourceFilesAreASCII` fails `make check` on a stray `—` or `§` in code. Escapes are fine; raw bytes are not.
- `make test` is plain `go test ./...`; **`make check` is the strict gate** — golangci-lint + `go test ./... -race` + oxlint + `tsc` + vitest + web build, mirroring `ci.yml`. Run `make check` before pushing, not just `make test`.
...
GitHub Flow: branch → PR → **squash-merge** (one issue = one commit); `main` always releasable; **the human merge is the sign-off — an agent never merges, never enables auto-merge, and never pushes to `main`.** **Label every PR at creation** (`enhancement`/`bug`/`documentation`/`dependencies`) **and assign the PR to its milestone** — the auto-notes depend on both. **PR bodies close their issue with a `Closes #N` line** — GitHub auto-closes the issue at merge, so issue numbers never sit in the heading/title. Releases are **batched, tag-driven** SemVer pre-releases (push `vX.Y.Z-alpha.N`). **One migration file through all of `0.x`** — `internal/db/migrations/00001_schema.sql`, edited in place, never a second file before `v1.0.0` (ADR-025). It must always read as the complete current schema, and after pulling a schema change you run **`make db-reset`**, not `make migrate-up` — goose cannot see that a file it already applied has changed. **Bump the pinned `URUNI_TAG` before tagging.** Never tag a red `main`. `0.x` breaking changes ride minor bumps.
