...
describe('Home', () => {
  it('renders the balance hero from fund_total, formatted with formatIDR', async () => {
    ...
    render(<Home refetchKey="1" onReconcile={vi.fn()} onDues={vi.fn()} />)
  ...
  it('calls onReconcile when the reconciliation banner is activated', async () => {
    ...
    render(<Home refetchKey="1" onReconcile={onReconcile} onDues={vi.fn()} />)

    await userEvent.click(await screen.findByText(copy.reconciliation.matched))
    expect(onReconcile).toHaveBeenCalledTimes(1)
  })

  // The dues roster's entry point (M6.12) - one plain link, same
  // router-agnostic contract as onReconcile above.
  it('calls onDues when the dues status link is activated', async () => {
    vi.stubGlobal('fetch', stubHome())
    const onDues = vi.fn()
    render(<Home refetchKey="1" onReconcile={vi.fn()} onDues={onDues} />)

    await userEvent.click(await screen.findByRole('button', { name: copy.dues.entryLink }))
    expect(onDues).toHaveBeenCalledTimes(1)
  })
  ...
  it('refetches when refetchKey changes, so a fresh record shows up without a manual refresh', async () => {
    const fetchMock = stubHome()
    vi.stubGlobal('fetch', fetchMock)
    const { rerender } = render(<Home refetchKey="1" onReconcile={vi.fn()} onDues={vi.fn()} />)
    await screen.findByText(money(1_450_000))

    const callsBefore = fetchMock.mock.calls.length
    rerender(<Home refetchKey="2" onReconcile={vi.fn()} onDues={vi.fn()} />)
