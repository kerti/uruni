...
5. **Migrations** stay in the same PR as their feature. Through all of `0.x` there is exactly **one** file — `internal/db/migrations/00001_schema.sql`, edited in place, never a second one before `v1.0.0` ([ADR-025](./docs/ADR/025-one-migration-file-until-1.0.md)). It must always read as the complete current schema. Migrations are embedded in the binary — `serve` applies what's pending on boot, so an operator never runs a migration step.
6. **After pulling a schema change, run `make db-reset`.** goose records applied migrations by number, so it cannot tell that `00001` was edited — your database will sit on the old schema and fail in confusing ways (`no such column`, a `CHECK` firing on a row the schema plainly allows). Resetting is the normal loop, not a recovery step; there is no data to lose before `v1.0.0`.
7. **Touching SQL means regenerating.** Queries live in `internal/store/queries/*.sql`; `make sqlc` regenerates `internal/store/`, and **the generated code is committed** so a reviewer sees exactly what the generator produced. sqlc is a separate install — `make doctor` says whether you have it. Name every projected expression (`SELECT 1 AS ok`, not `SELECT 1`): sqlc's SQLite engine mangles unnamed ones.
