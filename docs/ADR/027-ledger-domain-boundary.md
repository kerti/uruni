...
The package as it shipped: `ledger.go` (the type and the transaction helper), `transaction.go`, `transfer.go`, `reimbursement.go`, `incidental.go`, `reconciliation.go`, `dues.go`, `dues_status.go`, `balance.go`, `errors.go`. One file per operation, which is an implementation detail rather than a decision — it is recorded because a reader arriving at this ADR should not have to guess where anything lives.
...
`CloseIncidentalAndRoll` posts the `reclass_purpose` pair and sets `incidental.closed_on` inside a single `withTx`; a leftover of zero closes it and posts nothing, returning `0` and no error, because an error inside `withTx` would roll the close back and nothing asks for an envelope that landed square to stay open. ~~A negative leftover — an envelope that disbursed more than it collected — closes it and posts nothing too.~~ **Superseded by [ADR-031](./031-posting-to-a-closed-incidental.md)**: a negative leftover now covers the shortfall from the fund's main purpose in the opposite direction instead, so every close leaves the envelope's purpose balance at exactly zero rather than merely closing it while over-disbursed. The one refusal is `ErrIncidentalAlreadyClosed`: a second roll would move money that already moved. ~~and the purpose it closes stays open to new postings even afterward, so the check is the entire guarantee rather than a pre-check ahead of something the schema also enforces.~~ **Superseded by [ADR-031](./031-posting-to-a-closed-incidental.md)**: `PostTransaction` now refuses any posting to a closed envelope's purpose, in either direction (`ErrIncidentalClosed`), and reopening it (`POST /api/incidentals/{purposeID}/reopen`) is the deliberate, visible way back. PRD §7.5 describes one gesture ("on close, show the leftover and offer a one-tap roll into Kas Utama"), and splitting it across two calls means a crash between them leaves an envelope rolled but open — a state nothing in the schema forbids and nothing in the UI expects. The cost is that this package no longer *only* inserts: `incidental` is the one table M2 deliberately left without an immutability trigger, precisely because closing a collection moves no money and is a decision that gets revised, so an `UPDATE` here trips nothing and contradicts no non-negotiable.
...
**Everything else is not pre-validated or translated.** Composite-FK violations, an account that belongs to another fund, an unrecognized `kind` — none of it is checked before the write, because the IDs involved arrive from earlier `Querier` calls the domain itself made (an `account_id` from `ListAccountsByFund`, a `purpose_id` from `ListPurposesByFund`). A violation there is a domain bug, not a caller mistake, and surfaces wrapped generically (`fmt.Errorf("posting transfer leg: %w", err)`) for M4 to map to a 500 and a log line. **Except where an exported method carries a caller-supplied id into a write** — `UpdateReimbursement`'s `member_id` and `purpose_id` arrive from a `PATCH` body, not from an earlier `Querier` call, so a foreign-key violation there is the caller's typo and M4 maps it as one; see the Amendments below. **Immutability-trigger `RAISE(ABORT, …)` errors are unreachable from this package's own API**, because the only `UPDATE`s in the package are `CloseIncidentalAndRoll`'s and `UpdateReimbursement`'s, and the only `DELETE` is `DeleteReimbursement`'s, all against the two tables that carry no such trigger; every other write is an `INSERT` (`CLAUDE.md` rule 3) — so there is no code path here that could trip `transaction_immutable_update` or its siblings, so no translation is written for a message this package can never provoke. (Those triggers are, and stay, exercised directly against raw SQL in `internal/db`'s schema tests — that is what proves the database itself refuses the mutation, independent of whether `internal/ledger` ever asks for one.)
...
**2026-08-27 ([#103](https://github.com/kerti/uruni/issues/103))** — two sentences in *Everything else is not pre-validated or translated* went stale when correcting, waiving and deleting a reimbursement claim landed.

The first said "the only `UPDATE` in the package is `CloseIncidentalAndRoll`'s, against the one table that carries no such trigger; every other write is an `INSERT`." There are now two `UPDATE`s and one `DELETE`, against `incidental` and `reimbursement`. The conclusion is unchanged and was the point: neither table carries an immutability trigger, so no code path in this package can trip one.

The second sent every schema violation from the ledger to a 500, reasoning that "the IDs involved arrive from earlier `Querier` calls the domain itself made." `UpdateReimbursement` broke that premise — a `PATCH` body carries `member_id` and `purpose_id` straight from the client into the write — so a foreign-key violation there is a caller mistake, and `mapLedgerError` now classifies wrapped SQLite constraint errors instead of blaming the server for a typo. The general policy stands: the domain still does not pre-check ids the schema already refuses.

**2026-09-10 ([#230](https://github.com/kerti/uruni/issues/230))** — `PostOpeningBalance` and its `ErrOpeningBalanceExists` sentinel are gone. #230's uniform rule ("a location and its opening balance are born together, in one database transaction, or not at all") removed the standalone posting path entirely: an opening balance now only ever posts through `SetUpFund` or the new `Ledger.CreateAccount`, each inside the same `withTx` that creates the account it belongs to. Neither needs `ErrOpeningBalanceExists` — a brand-new account cannot already carry an opening row — so the schema's `opening_balance_once_per_account` partial unique index is now the only guarantee, with no Go pre-check sitting on top of it. The general shape this ADR describes is unchanged: one package, one transaction helper, argument-shape failures rejected before `withTx` opens.
