...
**Iuran loses its slot** and becomes a tab under Riwayat. The tab is the dues payment history, a list like its three siblings; the status matrix is its own screen at `/dues`, opened from a row at the top of the tab. This is the cost of coherence and it was taken deliberately: dues status is one reading of one month, and it was occupying a fifth of the app's navigation. An earlier draft put the matrix at the top of the tab itself; #228 showed a monthly reading stacked over an unbounded list reads as clutter, and it was the one panel breaking this ADR's own rule that tabs hold lists.
...
It belongs with Titipan instead, because they are the same object. [`CONTEXT.md`](../../CONTEXT.md) makes **incidental** and **pass-through** two kinds of one `purpose`, and adding a pass-through is already a Pengaturan section. So the app teaches one rule rather than two: **a purpose is created in Pengaturan, then it appears in Beranda's breakdown and money flows through Catat.** An incidental is that, with a lifecycle attached — and the lifecycle acts (contribute, disburse, close, reopen) stay where they already are, on the envelope itself.

**The `/incidentals` list screen is retired by the same stroke.** Its card list *is* the Pengaturan section, so keeping both would be the duplication this ADR exists to remove; and its open/all tabs were component state, which the tabs-are-routes rule above does not allow anyway. What survives is the detail view at `/incidentals?purpose=<id>`, reached from a Beranda row and from a Pengaturan card. The section lists closed envelopes as well as open ones, which is what gives reopening ([ADR-031](./031-posting-to-a-closed-incidental.md)) a door that does not wait on [#262](https://github.com/kerti/uruni/issues/262).
...
**Tier management leaves Anggota for Pengaturan.** This supersedes the placement in M6.16/M6.17, whose stated reason was that *"a member's tier is set on the member, so the two are read and edited in one sitting."* That premise stops holding here, because the roster row now **displays** the tier's effect (below) — the two are read together on the roster itself and written together only rarely. (Shipped in that order and not the other: [#232](https://github.com/kerti/uruni/issues/232) moved the section in M6.31, [#233](https://github.com/kerti/uruni/issues/233) adds the display in M6.32. Between the two, Anggota shows the tier name it always did — the move costs the roster nothing it had.) Anggota gets its full height for one long, searchable, paged list; Pengaturan gains a genuinely settings-shaped thing; and the app keeps exactly one screen where tabs mean anything.
...
| Dialog | Route |
|---|---|
| rename fund · add/rename/retire location · add/edit pass-through purpose · add/edit member · add/edit tier · add/edit rate · open an incidental · rename an incidental · **correct a peruntukan** | record transaction · transfer · dues payment · reimbursement claim · reconcile count |
...
**The rule is narrower than "posts no ledger entry", and this is the wording** (revised 2026-09-18, [#276](https://github.com/kerti/uruni/issues/276)): **a dialog may post only when the posting is value-neutral and carries a single field.** Correcting a peruntukan ([ADR-033](./033-correcting-a-posted-purpose.md)) posts two legs, so the original phrasing would have made it a route - but the pair cannot move a balance in any month, and the body carries one field, the target tag. The fear this rule was written about is named below: "a posting dismissible by a stray backdrop tap is the wrong shape for the one thing this app must not get wrong." A correction dismissed by a stray tap does nothing at all. The same sentence readmits the opening-balance exception stated further down, which stops being a special case and becomes an instance, and it still refuses every money form.

**The control is the peruntukan itself, not the row** ([#276](https://github.com/kerti/uruni/issues/276)). Transaction rows are interactive for the first time here, and the affordance is deliberately the smallest one that works: eligibility varies row by row, so a tappable row would be a dead tap on a dues row and a live one on the expense beneath it with nothing to tell them apart, while a button of its own would either break the amount column's right alignment - the column she scans - or add 44px to every eligible row on the app's densest unbounded list. A second action on a row becomes a second control; a third earns a `(...)` menu, since a menu holding one item is an extra tap and a hidden label for no gain. [#212](https://github.com/kerti/uruni/issues/212) inherits that, rather than meeting it cold.
...
**A section clears only the values it owns** (added 2026-09-16, after [#263](https://github.com/kerti/uruni/issues/263) broke this in e2e). Pengaturan renders every section against one URL, so once more than one of them owns an `?edit=` dialog they all read the same param. A section that strips whatever it cannot parse will therefore strip the value its neighbour's open dialog is living on and close it mid-edit - racily, depending on whose list resolves first. So "not mine" and "mine but malformed" are different answers: a foreign prefix is left untouched, and only a value carrying this section's own prefix is ever cleared. Section-level tests cannot see this - a section mounted alone has nobody to collide with - so the guard is a test that mounts Pengaturan whole (`screens/Settings/index.test.tsx`).
