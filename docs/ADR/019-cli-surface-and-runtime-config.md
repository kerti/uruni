...
| `URUNI_UPLOADS_DIR` | `./uploads` | Local volume for uploaded receipt photos ([ADR-011](./011-receipt-photos-local-volume.md)). The default mirrors `URUNI_DB`'s own split - dev-friendly and relative, overridden to the volume-mounted `/uploads` explicitly in `docker-compose.yml`. Checked at boot, for `serve` only: the directory must already exist and accept a new file, or the server refuses to start (`config.EnsureUploadsDirWritable`) - the same "seeded root:root" failure this ADR's own Consequences paragraph already describes for `/data` and `/uploads`, turned into a named boot error instead of a receipt upload failing later. |
...
**2026-09-26 ([#153](https://github.com/kerti/uruni/issues/153))** — the runtime-config table gained a `URUNI_UPLOADS_DIR` row: the local volume [ADR-011](./011-receipt-photos-local-volume.md) already named in its own decision, never before read by any code. Recorded the same way the row above was: on the maintainer's call, since restating the whole table to add one row is the same ceremony the prior amendment already declined to perform for deleting one. The decision this ADR makes is unchanged — this is one more variable in the same env-vars-only table, parsed by the same one `Load`, with a boot-time existence-and-writability check of its own (`config.EnsureUploadsDirWritable`) that runs only for `serve`, the only subcommand that ever writes a receipt.
