...
No `Neg()` — `-a` is exact for every representable `Amount` except negating `math.MinInt64`, and the ledger cannot hand Go that value in the first place: SQLite's `SUM()` **raises `integer overflow` as a genuine runtime error** when a sum exceeds `int64`, rather than silently widening to a float (verified against `modernc.org/sqlite`, the driver this project actually uses — note that `total()` *does* do the float-silent thing, which is one reason no query here uses it). A balance that large fails as a query error long before it reaches an `Amount`. No comparison methods — `a < b` already compiles and is correct.
