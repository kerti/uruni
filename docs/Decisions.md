...
## One instance, one treasurer, one fund — and the window that closes at 1.0 (decided 2026-08-27)

[ADR-030](./ADR/030-multi-fund-scoping.md) settles [#105](https://github.com/kerti/uruni/issues/105) and carries the architecture. Three things sit above it and outlive the slice.

**Multi-tenancy is a scope call, and it was made against §8, not §4.** A maintainer question during the grill asked for one instance serving two treasurers — a spouse's work unit alongside a community's kas. That is not [#79](https://github.com/kerti/uruni/issues/79)'s one-treasurer-several-funds: #79 needs fund *selection*, this needs fund *authorization* on every read and write, plus everything #79 already lists. Filed as its own post-1.0 issue, [#110](https://github.com/kerti/uruni/issues/110), deliberately not a comment on #79, because merged they would get the wrong one built. Note what forbids it: **not** a `PRD.md` §4 non-goal — that bullet is about *members* not having logins — but §8's "local auth for the single treasurer in v1" and the prime directive. The answer today is a second instance; SQLite and one binary make that nearly free, and a fault in one cannot reach the other community's money.

**The deferral is cheap only until `v1.0.0`.** [ADR-025](./ADR/025-one-migration-file-until-1.0.md)'s edit-in-place migration ends at the tag by definition, so the user↔fund relation ADR-030 declines to build is free to add only while no treasurer has data underneath it. That makes the tag itself a decision point: #110 and #79 get an explicit yes-or-no from what the pilot at `v0.6.x` actually showed **before** `v1.0.0` is pushed. The answer may well still be no — it should be a decision rather than a silence.

**One-shot registration buys the lock and costs recovery.** Registration precedes setup and refuses the moment *any* account exists — the refusal is keyed to the count, not to a duplicate identifier, because a stranger with a different email would otherwise sail past it and `resolveFund` would hand them `funds[0]`. The accepted cost is that a forgotten password has no self-service path (one account, and §8 data minimization means no address to mail). The host resets it at the database or by CLI. A `make` target closes this cheaply whenever it is worth doing — plausibly an **M10**, since `ROADMAP.md`'s M9 is the last milestone listed, not necessarily the last one before `v1.0.0`.

## The gate closes, and a library gets bought rather than written (decided 2026-08-31)

M5's last slice ([#116](https://github.com/kerti/uruni/issues/116)) closed `/api` and dropped the `draft` tags on [ADR-007](./ADR/007-auth-local-email-password.md) and [ADR-030](./ADR/030-multi-fund-scoping.md). Three things outlive the slices.

**A dependency was the cheaper side of rule 7 here, not the expensive one.** ADR-007 had carried an open choice since M1 — scs "or a minimal hand-rolled store given it's effectively one user" — and the parenthetical read as licence to skip the dependency. It is resolved to **scs**, with storage in our own `Store` over sqlc rather than scs's bundled `sqlite3store`, which keeps every query inside [ADR-005](./ADR/005-data-access-sqlc.md)'s reviewed-SQL discipline. "Self-host simplicity wins ties" is about moving parts an operator has to run; a linked MIT library is not one. The risk being hedged — session fixation, non-constant-time token comparison, cookie-flag mistakes — is the class `CLAUDE.md` sends to `builder-deep`, and "effectively one user" reduces none of it.

**No CSRF token, and it is written down so its absence reads as a decision.** Single origin, a JSON API, and `SameSite=Lax` between them mean no cross-site context attaches the session cookie to a state-changing POST. A token would add a round trip and a failure mode to defend a seam already closed. A real cross-origin client reopens this.

**ADR-030 said `POST /logout` sits inside the gate. It cannot.** Logout is idempotent by that same paragraph's ruling — 204 whether or not a session was there — and a route behind `sessionRequired` answers 401 to precisely the caller whose request is already satisfied. The two halves of that sentence contradicted each other, and the placement half was wrong; corrected in place while the ADR was still `draft`, in the PR that made it true. Worth recording because it is the failure mode `draft` exists to catch: prose that sounds settled because nothing had yet tried to run it.

## The frontend gets a router, and it is a bought one (decided 2026-09-02)

M6.2 ([#135](https://github.com/kerti/uruni/issues/135)) needed client-side routing, which meant the first new frontend dependency since the M1 scaffold — the case rule 7 asks to be named rather than assumed.
...
## An incidental is a purpose, so it is opened in Pengaturan (decided 2026-09-16)

[#265](https://github.com/kerti/uruni/pull/265) made open envelopes entry points in Beranda's purpose breakdown and removed the button that used to reach the incidentals screen — correctly, per [ADR-032](./ADR/032-two-level-navigation.md), which says it "removes a button from Beranda rather than adding one". But the ADR only settled where an envelope is *read*. Nothing then reached the open form, so on the first pilotable build a treasurer could not start a kegiatan insidental at all ([#263](https://github.com/kerti/uruni/issues/263)).

**The ruling: a new envelope is opened from a Pengaturan section, beside Titipan.** The reason is that they are the same object — CONTEXT.md makes incidental and pass-through two kinds of one `purpose`, and the app already adds a pass-through there. One rule instead of two: a purpose is created in Pengaturan, then it shows up in Beranda's breakdown and money moves through Catat. Beranda was rejected on its own merits as well as the ADR's: the breakdown section renders nothing when no envelope is open, which is exactly the state she is in when she wants her first one.

**The `/incidentals` list screen is retired with it** — the Pengaturan card list replaces it, and only the `?purpose=<id>` detail view survives. The section lists closed envelopes too, so reopening keeps a door without waiting on [#262](https://github.com/kerti/uruni/issues/262)'s purpose filter.
...
## A closed envelope's rollover is derived, never stored (decided 2026-09-17)

The rollover lived only in the close response, held in component state, so it was readable for one screen-lifetime and absent on every visit after — [#270](https://github.com/kerti/uruni/issues/270). A treasurer returning to a closed envelope saw Terkumpul Rp10.000 against Terpakai Rp0 on an envelope holding nothing, and no sentence reconciling them.
