...
**Status:** **M6 — everyday UI** is in progress: planned as twenty-two slices, now thirty issues, nine of them open. `/api` has been complete and session-gated since `v0.5.0`; what M6 adds is the screen the ledger has never had — record → home → reconcile. It grew because building that screen kept finding things: [ADR-032](./ADR/032-two-level-navigation.md) replaced the navigation wholesale and brought twelve slices with it, and walking the app on a phone surfaced the rest. The slice count is a record of that, not an overrun — but the alpha count that used to sit in this sentence was arithmetic over the old plan, so it is gone rather than restated.
