...
- Navigation is a **sticky bottom nav** with five destinations — Beranda · Riwayat · Catat · Anggota · Pengaturan ([ADR-032](./ADR/032-two-level-navigation.md)). Catat is a raised Forest circle that never changes; the active slot, Catat included, gets a Forest line on the bar's top edge plus Forest icon and caption, `aria-current` alongside; resting slots sit at 80% muted. Every caption shares one baseline. Five is the platform's own cap for a tab bar: at 375px that is 75px a tab, which fits an 11px label. A sixth would mean a different pattern, not a narrower tab. The header's fund name is a second way home — never the only one, since a top-corner control is the hardest thing on a phone to reach one-handed. It replaced the circular add-FAB at M6.15: "catat" is a tab now, and no screen has two entry points. Reconcile is deliberately not a tab — the reconciliation banner on home is its affordance.
- **Action rows: the buttons that end a screen or a form share one row, primary on the right.** Two fit comfortably at 375px; three is the cap. Stacking them full-width turns a two-choice decision — save or cancel — into what reads as a list of things to do, and pushes the primary action below the fold on the taller forms. Each button still clears 44px; the row is a `grid` so the options divide the width evenly rather than sizing to their captions, which keeps `Batal` from looking like an afterthought beside `Simpan`.
  **Past three, stop and ask why the screen has four decisions**, rather than reaching for a second row. A fourth button is usually a navigation dressed as an action (it belongs in the body, as a link or a card), or a sign the screen is doing two jobs. Two rows are the escape hatch, not the pattern — and a dialog's own footer is the same rule with a harder cap, since ADR-032 gives it exactly two.
